...
MindsDB documentation. You can install MindsDB in
...
You can use the MindsDB Cloud SQL Editor or open your preferred SQL client, such
as DBeaver or MySQL CLI, and connect to MindsDB.

<Tabs>

{" "}
<Tab title="Using the MindsDB Cloud SQL Editor">
...
## 3. Connecting a Database Using [`CREATE DATABASE`](/sql/create/databases/)
...
## 5. Creating a Model Using [`CREATE MODEL`](/sql/create/predictor/)

Now you are ready to create your first predictor. Use the
...
It may take a couple of minutes until the predictor is trained. You can monitor
the status of your predictor by executing the following command:
...
SHOW MODELS
FROM mindsdb
LIKE 'home_rentals_predictor'
WHERE status='complete';
```

<Warning>
  The status of the predictor must be `complete` before you can start making
  predictions.
</Warning>

## 7. Making a Prediction Using [`SELECT`](/sql/api/select/)
...
<Check>
  **Congratulations!** If you got this far, you have successfully trained a
  predictive model using SQL and got the future data!
</Check>
